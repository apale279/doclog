...
export function TipiLuogoChipsEditor() {
  ...
  return (
    <section className="rounded border border-slate-300 bg-white p-4">
      <h3 className="mb-3 text-sm font-bold uppercase text-slate-800">Tipo luogo</h3>
      <form onSubmit={addTipo} className="mb-3 flex flex-wrap gap-2">
        <input
          className={`${inputClass} min-w-[160px] flex-1`}
          value={nuovo}
          onChange={(e) => setNuovo(e.target.value)}
          placeholder="Nuovo tipo luogo…"
          disabled={saving}
        />
      ...
      {list.length === 0 ? (
        <p className="text-sm text-slate-500">Nessun tipo luogo definito.</p>
      ) : (
        <ul className="flex flex-wrap gap-2">
          {list.map((tipo) => (
            <li key={tipo}>
              <span className="inline-flex items-center gap-1 rounded-full border border-teal-200 bg-teal-50 py-1 pl-3 pr-1 text-sm font-semibold text-teal-900">
                {tipo}
                <button
                  type="button"
                  className="rounded-full p-0.5 text-teal-600 hover:bg-red-100 hover:text-red-700"
                  onClick={() => remove(tipo)}
                  disabled={saving}
